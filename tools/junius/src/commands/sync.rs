...
pub fn run(args: &SyncArgs, format: OutputFormat) -> i32 {
    ...
    // Per-plugin RPC barrel files under packages/generated/src/plugins/<name>/.
    // The actual proto-to-TS codegen (packages/generated/src/proto/) is
    // produced by `buf generate`; the barrel re-exports the parts that
    // plugins should use as their public RPC surface. (Narrowing per
    // [dependencies.<dep>].rpc_methods arrives in M09.)
    for plugin in &plugins {
        let path = rpc_barrel_path(&plugin.name);
        let content = render_rpc_barrel(plugin);
        match apply_file(&path, &content, args.dry_run) {
            Ok(kind) => changes.push(FileChange {
