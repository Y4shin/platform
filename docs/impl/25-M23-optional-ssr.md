...
> embedded. The SSR framework spike picked **hand-rolled React 19
> `renderToPipeableStream` + TanStack Router** over TanStack Start
> (see Library choices below). Stage 2 (SSR FE host package
> scaffolding — `platform/frontend-ssr/`, hand-rolled SSR + hydration
> entries, Node server with Vite middleware-mode dev + prod
> serve-from-bundle) landed too. Stages 3–6 are next: API base URL
...
|---|---|---|---|
| SSR framework | **Hand-rolled React 19 `renderToPipeableStream` + TanStack Router SSR primitives** | Spike findings: our `routeTree` is already code-generated by `junius sync` (`platform/frontend/src/generated/routes.ts`) using `createRoute`/`addChildren`; TanStack Start's default convention is file-based routing (`src/routes/` auto-scanned by `@tanstack/router-plugin`), which fights the plugin-contributed-routes contract. Start's pipeline is also heavy (22+ deps incl. babel, rolldown, lightningcss, router-generator) and on a fast-moving 1.168.x minor cadence. Hand-rolled is ~100 LOC + a Vite SSR config and tracks the more stable TanStack Router 1.x. | M24 (`frontend` image) |
| Alternative considered | TanStack Start with code-based routing | Rejected during the M23 spike — non-default path on a still-young framework; the cost of fighting file-based conventions outweighed the framework's batteries-included server functions. | — |
