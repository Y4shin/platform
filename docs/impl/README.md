...
| **M18** | 🚧 | [Group & Role Provisioning](20-M18-group-role-provisioning.md) | User-roles (incl. built-in `admin` with `*` permission); `admin` plugin (groups/roles/permission editing + OIDC mapping UI); OIDC `groups` claim reconciled at login + self-refresh endpoints (REST + Connect) + `junius oidc resync` CLI; declarative TOML provisioning auto-applied at boot (hash-guarded) with optional `lock_managed` making config rows read-only at every mutation seam | A fresh deployment can create groups/roles/permissions in the browser, OIDC group memberships sync automatically from Authentik, and `dev-seed.sql` is replaced by `dev/provisioning.toml` |
