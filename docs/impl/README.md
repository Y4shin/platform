...
| **M17** | ⏳ | [E2E testing](19-M17-e2e-testing.md) | Per-plugin Playwright specs (`plugins/*/frontend/e2e/**`) auto-discovered by `task test:e2e`; `@junius/e2e` fixtures with session-seed login (no Authentik UI); ephemeral testcontainers stack; a sixth CI job. Code landed; first green run blocked on a pre-existing `@lingui/vite-plugin` build error (see M17 doc). | Drop a plugin spec → it runs; `task test:e2e` provisions + logs in unattended; CI runs E2E green/red |
