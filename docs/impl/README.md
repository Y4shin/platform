...
| **M14** | ⏳ ★ | [Internationalization](16-M14-internationalization.md) | Seam landed (Stages A–E + CI gate): host-side locale storage, typed-codegen backend localizer, FE locale Context/persistence, `junius i18n check`, end-to-end retrofit of `hello`. Lingui FE integration + the other three plugins' retrofit are the open follow-ups. | `task ci:i18n` green; `hello` job test renders the email subject in EN + DE |
...
| **M17** | 🚧 | [E2E testing](19-M17-e2e-testing.md) | Per-plugin Playwright specs (`plugins/*/frontend/e2e/**`) auto-discovered by `task test:e2e`; `@junius/e2e` fixtures with session-seed login (no Authentik UI); a fifth CI job | Drop a plugin spec → it runs; `task test:e2e` provisions + logs in unattended; CI runs E2E green/red |
