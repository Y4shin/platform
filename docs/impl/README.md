...
| **M18** | ⏳ | [Group & Role Provisioning](20-M18-group-role-provisioning.md) | User-roles (incl. built-in `admin` with `*` permission); `admin` plugin (groups/roles/permission editing + OIDC mapping UI); OIDC `groups` claim reconciled at login + REST `/api/me/refresh-groups`; declarative TOML provisioning (`junius provision apply`, hash-guarded) with boot-time auto-apply and `lock_managed` enforcement at the `PlatformAdminApi` seam. **Deferred:** `junius oidc resync` CLI; Connect-RPC self-refresh. | A fresh deployment can create groups/roles/permissions in the browser; OIDC group memberships sync at login; `dev/dev-seed.sql` is replaced by `dev/provisioning.toml` |
...
| **M22** | 🚧 | [RustFS evaluation](24-M22-rustfs-storage.md) | Spike RustFS as a lightweight Rust-native MinIO replacement for the dev / E2E / example stacks (production endpoints unchanged). Land a probe test against `rust-s3`'s actual API surface; either swap MinIO out everywhere or write up the specific "not yet" gap so the question stops being re-raised. | Probe test green against RustFS for every S3 call `platform/src/storage/` issues; if adopted, `task dev` + `task test:e2e` + the events storage demo all round-trip green against RustFS |
| **M23** ★ | ⏳ | [Optional SSR mode](25-M23-optional-ssr.md) | Second deployment topology: headless `juniusd` + a Node SSR FE container (TanStack Start) talking to the BE over a split client/server API base URL with per-request session-cookie forwarding so SSR runs as the requesting user. Embedded-SPA mode stays the default. | `e2e-split` CI job green; the new `examples/example-deployment-split/` boots both containers and SSRs bob's vs alice's event list correctly |
| **M24** ★ | 🚧 | [Precompiled containers](26-M24-precompiled-containers.md) | Three official images on `ghcr.io` (`full`, `frontend`, `backend`) bundling every monorepo plugin, built per push to `main`, tagged `<variant>-latest` + `<variant>-<sha>`, cosign-signed. Boot-time check enforces that `[plugins].enabled` matches the bundled set exactly; source-build path stays first-class for subsetting. `full` + `backend` also ship a trimmed `junius` CLI (migrate / check / list / i18n; no source-build commands). | `docker pull ghcr.io/.../junius-full:latest` + a stock `platform.toml` boots green; `junius migrate up` inside the container applies migrations; the precompiled-split example runs the M23 SSR Playwright slice |
...
**★ Top priority:** M00–M13 shipped English-only; M14 was the prioritized next milestone after M13. **M23 (Optional SSR) and M24 (Precompiled containers) now jump the queue ahead of M18–M22.** Both are deployment-shape changes that are cheaper to land before the in-progress UI/admin work (M18–M20) hardens around assumptions about the embedded-SPA topology. M23 first (M24's `frontend` + `backend` images depend on it); M24's `full` image is independent and could ship in parallel.
